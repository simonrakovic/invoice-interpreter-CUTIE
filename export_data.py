    ...
    for ticket_id in task_info:
        ...
        try:
            if r.ok:
                ...
                with open('{path}/{id}.modeled'.format(path=dir_name, id=ticket_id), 'w') as f:
                    f.write(json_result)

                print('Download: {file}.modeled'.format(file=ticket_id))
            else:
                print('Download file failed.')
                print('Call api: {url}'.format(url=api))
                print('[Err Reason]:{err}'.format(err=r.reason))
    ...
    for ticket_id in task_info:
        ...
        if r.ok:
            try:
                ...
                print('download label {file}.modeled failed'.format(file=ticket_id))
                print(str(e))
                continue
...
if __name__ == "__main__":
    ...
    headers = {
        'Content-Type': "application/modeled",
